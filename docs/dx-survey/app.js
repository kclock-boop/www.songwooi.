...
$('open-dashboard').addEventListener('click',e=>{
 e.preventDefault();
 try{sessionStorage.setItem('songwoo.dx-dashboard.transfer',JSON.stringify(payload()));dirty=false;window.location.href=e.currentTarget.href;}
 catch{status('대시보드로 응답을 전달하지 못했습니다. 응답 파일(JSON)을 저장한 뒤 대시보드의 불러오기를 사용해 주세요.');$('status').scrollIntoView();}
});
